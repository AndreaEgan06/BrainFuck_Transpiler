...
void process(char *code, int *codePtr, unsigned char *tape, int *index,
             char *input) {
  ...
  case '[':
    if (tape[*index]) {
      if (parenthesesStack[parenthesesStackCounter] != *codePtr)
        parenthesesStack[++parenthesesStackCounter] = *codePtr;
    } else {
      if (parenthesesStack[parenthesesStackCounter] == *codePtr)
        --parenthesesStackCounter;
      int parenCounter = 0;
      while (code[++(*codePtr)] != ']' || parenCounter > 0) {
        if (code[*codePtr] == '[') {
          ++parenCounter;
  ...
  case ']':
    if (tape[*index]) {
      *codePtr = parenthesesStack[parenthesesStackCounter] - 1;
    } else
      --parenthesesStackCounter;
    break;
  }
